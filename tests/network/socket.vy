...
dos_timer        :: Float64 = 0.0;                   # Active Screen Freeze Timer
...
fn load_page(url :: String) -> Array {
    ...
    if (clean_u == "shadow.dir") {
        return [
            ...
            "[LINK:market.vnet] -> The Red Market (Exploits & Stolen Keys)",
            "[LINK:crypto.vnet] -> Black Tumbler Wallet Ledger",
            "[TEXT] ",
            "[SUBTITLE] CATEGORY: RESTRICTED STREAMS & SURVEILLANCE",
            "[LINK:dollhouse.vnet] -> Live Surveillance Feed #0992",
            "[LINK:redroom.vnet] -> Encrypted Stream Node Alpha",
            "[TEXT] ",
            "[SUBTITLE] CATEGORY: DATA VAULTS & FORUMS",
            "[LINK:vault.vnet] -> Corrupted VFS Memory Dump /vault/sys/",
            "[LINK:forum.vnet] -> /b/ - Anonymous Board",
    ...
    if (clean_u == "forum.vnet") {
        return [
            ...
            "[HR]",
            "[TEXT] Ghost_User: I found key 5 buried in this signature [7712].",
            "[LINK:shadow.dir] << RETURN TO DIRECTORY"
        ];
    }
...
fn dispatch_cli_command(raw_input :: String) {
    ...
    if (cmd == "netscan") {
        ...
        if (args == "") { cli_logs.push("[ERROR]: Usage: dos <target_port>"); }
        else { vnet.send_to(client_sock, server_ip, server_port, "DOS:" + args); }
    }
    else if (cmd == "redirect") {
        # Format: redirect <port> <url>
        ...
        if (args == "") { cli_logs.push("[ERROR]: Usage: snoop <target_port>"); }
        else { vnet.send_to(client_sock, server_ip, server_port, "SNOOP:" + args); }
    }
    else if (cmd == "spike") {
        if (args == "") { cli_logs.push("[ERROR]: Usage: spike <target_port>"); }
        else { vnet.send_to(client_sock, server_ip, server_port, "SPIKE:" + args); }
    }
    else if (cmd == "scan") {
        ...
        cli_logs.push("========== P2P OFFENSIVE HACKING COMMANDS ==========");
        cli_logs.push("  netscan                 - Discover active peers on current URL");
        cli_logs.push("  dos <port>              - Freeze peer browser & trigger screen glitch");
        cli_logs.push("  redirect <port> <url>   - BGP Hijack peer browser to forced URL");
        cli_logs.push("  snoop <port>            - Interrogate target live URL location");
        cli_logs.push("  spike <port>            - Force +35% threat trace spike on peer");
        cli_logs.push("========== GATEWAY & LOCAL SYSTEM COMMANDS ==========");
        cli_logs.push("  scan                    - Interrogate gateway firewall & salt");
        cli_logs.push("  crack <val>             - Submit hash breach payload");
        cli_logs.push("  cat <vfs_path>          - Exfiltrate remote system file");
        cli_logs.push("  patch                   - Relock firewall & rotate salt");
...
while (vglib.running()) {
    ...
    # UDP PACKET INGEST & EXPLOIT HANDLER
    ...
    if (packet_in.length() >= 3) {
        ...
        # INBOUND EXPLOIT PACKET HANDLERS
    ...
    vglib.begin();
        ...
        vglib.text_ex(vcr_font, "- NODE " + string(my_port) + " ACTIVE", 945 + jitter_x, 245, 10, COLOR_GHOST);
        ...
        through line_item :: page_body -> loop {
            ...
            if (y_pos >= 85.0 && y_pos <= 730.0) {
                if (line_str.length() > 7 && line_str.substr(0, 7) == "[TITLE]") {
                    ...
                    is_hover :: Int64 = (cli_overlay_open == 0 && dos_timer <= 0.0 && mx >= 40.0 && mx <= (40.0 + lbl_w + 20.0) && my >= y_pos && my <= (y_pos + 22.0)) ? 1 : 0;
        ...
        # REAL-TIME CLI TERMINAL OVERLAY
        if (cli_overlay_open == 1) {
